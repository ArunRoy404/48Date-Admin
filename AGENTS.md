...
- **Never import a `ui/*` primitive directly into a page or feature component.** Customize the primitive itself (its `cva` variants in `ui/button.tsx`, `ui/input.tsx`, etc.) so its *default* look already matches Figma, and/or wrap it in a `common/` component for anything with app-specific behavior (`CommonButton`, `CommonInput`, `CommonOTPInput`, `UserAvatar`, `StatusBadge`, `PageHeader`, `Logo`, `SearchInput`, `UserMenu`). Feature code imports from `common/` or the customized `ui/*`, never a raw unstyled primitive or native element.
- **Forms & Inputs**: use `CommonInput` (`src/components/common/CommonInput.tsx`) for every text/email/password/textarea field — pass `type`. Password show/hide state lives inside `CommonInput`, not in the parent. Use `CommonOTPInput` for any digit-code input.
- **Buttons**: use `CommonButton` (`src/components/common/CommonButton.tsx`) or customized shadcn button variants — never raw `<button>` tags with hand-rolled styles in pages.
- **Charts & Data Visualizations**: Always use **Recharts** (`recharts`) for all charts (AreaChart, BarChart, PieChart, Donut, etc.) wrapped inside `ResponsiveContainer`. Never hand-roll raw SVG coordinate charts. Style them with design tokens from `globals.css` and custom tooltips.
- **Dummy Data & State Management with Zustand**:
  - **Never define any hardcoded or dummy data inside components or pages.**
  - All mock data, static lists, mock metrics, and content MUST live under `src/dummyData/` organized by page/section/domain (e.g. `src/dummyData/stats.ts`, `src/dummyData/userGrowth.ts`, `src/dummyData/matchStatus.ts`, `src/dummyData/revenue.ts`, `src/dummyData/activity.ts`, `src/dummyData/navigation.ts`, `src/dummyData/user.ts`).
  - State management uses **Zustand** (`zustand`). Stores live under `src/store/` (e.g. `useDashboardStore.ts`, `useNavigationStore.ts`, `useUserStore.ts`).
  - Components and pages must consume data, metrics, and state **exclusively via Zustand store hooks**.
  - Navigation and routing data must live in separate files under `src/dummyData/` and be accessed via store hooks.
...
- **Routing**: the dashboard root overview page lives in `src/app/dashboard/(overview)/page.tsx`. Other subpages follow standard named routes (e.g., `src/app/dashboard/users/page.tsx`).
- **Reuse, don't duplicate.** Before adding a new status-color map, badge variant, or avatar treatment, check `src/components/common/` first — these exist specifically to eliminate duplication.

## Never duplicate code — extract a reusable component

If you are about to paste, retype, or closely re-derive a block of JSX/markup that already exists elsewhere in the codebase (even with different text/props), stop and extract it into a reusable component instead. This applies at any scale — a whole page section, a form-field group, a title+description header block, a status badge, a card shell. `AuthCardHeader`, `PageHeader`, `StatusBadge`, `SearchInput`, `UserMenu`, `Logo`, `CommonButton`, and `FilterTabs` all exist because the same markup was found copy-pasted across 2+ files; that duplication should have been caught before it landed. Before writing new markup:
